...
# ========== دالة DeCode_PackEt النهائية لـ xHeaders.py ==========
def DeCode_PackEt(input_text):
    """
    فك تشفير حزمة Protobuf وإرجاع JSON string
    """
    try:
        from protobuf_decoder.protobuf_decoder import Parser
        parser = Parser()
        
        # input_text هو نص عادي من binascii.hexlify
        if isinstance(input_text, str):
            ...
            try:
                ...
                return json.dumps({"error": "invalid hex"})
        else:
        ...
        for item in parsed_results.results:
            ...
            if item.wire_type == 'bytes' and hasattr(item.data, 'results'):
                # حقل متداخل
                result[field_num] = str(item.data)
            elif item.wire_type == 'varint':
                result[field_num] = item.data
            elif item.wire_type == 'string':
                result[field_num] = item.data
            elif item.wire_type in ['fixed32', 'fixed64']:
                result[field_num] = item.data.unsigned_int if hasattr(item.data, 'unsigned_int') else str(item.data)
            else:
                result[field_num] = str(item.data)
        
        # نلف النتيجة في {'data': ...} ثم نحولها لـ JSON
        final_result = {}
        for key, value in result.items():
            final_result[key] = {'data': value}
        
        return json.dumps(final_result)
        
    except Exception as e:
        print(f"⚠️ خطأ في DeCode_PackEt: {e}")
        return json.dumps({})